...
#if (! defined HOST) && (! defined EXTRUDER)
	#error: building for host or extruder? define in makefile, -DHOST or -DEXTRUDER in cflags
#endif
...
intercom_state_e state;
uint8_t cmd, send_cmd, read_cmd;
...
#endif
{
	...
	if (state >= READ_START1) {
		switch(state) {
			...
			case SEND_CHK:
			case SEND_DONE:
				// this is mostly just to keep the compiler happy, but if we ever actually get here something is wrong
				break;
		}
	}
}
